...
class SmartPricePredictionPipeline:
    ...
    def __init__(self):
        ...
        if luxury_model_path.exists():
            ...
            print(
                "[WARNING] Luxury estate model not found, will use standard model only")
    ...
    def predict_price(self, address: str, real_features: dict = None) -> dict:
        ...
        if real_features:
            ...
            property_data = AssessorAPIConnector.search_property_by_address(
                address)
        ...
        print(
            f"  Lot Size: {property_data.get('LotArea', 0):,.0f} sq ft ({property_data.get('LotArea', 0) / 43560:.1f} acres)")
        ...
        if is_luxury and self.luxury_model:
            ...
            print(
                f"  Luxury Tier: {enhanced_features.get('LuxuryTier', 'Unknown')}")
            print(
                f"  Land-to-Building Ratio: {enhanced_features.get('LandToBuilding', 0):.1f}")
            print(f"  Has Pool: {enhanced_features.get('HasPool', 0)}")
            print(
                f"  Has Tennis Court: {enhanced_features.get('HasTennisCore', 0)}")
            print(
                f"  Has Guest House: {enhanced_features.get('HasGuestHouse', 0)}")
            ...
            result = self.standard_pipeline.predict_price(
                address, real_features=property_data)
...
def test_smart_pipeline():
    ...
    print("-"*75)
    print(
        f"{'Price':<35} ${result['predicted_price']:>18,.0f} ${ACTUAL_REDFIN_PRICE:>18,.0f}")
    ...
    print(
        f"  School District: {result['school_district']} ({result['school_rating']:.1f}/10)")
    ...
    new_error_pct = abs(
        (ACTUAL_REDFIN_PRICE - result['predicted_price']) / ACTUAL_REDFIN_PRICE * 100)

    print(f"\nPrevious model error: {old_error_pct:.1f}%")
    print(f"Luxury model error:   {new_error_pct:.1f}%")
    print(
        f"Improvement:          {old_error_pct - new_error_pct:.1f} percentage points")
